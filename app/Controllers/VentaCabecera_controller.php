...
class VentaCabecera_controller extends Controller{
    ...
    private function filtrarComprasCliente(?string $queryFechaInicio, ?string $queryFechaFin){
        ...
        if($queryFechaInicio && $queryFechaFin && ($queryFechaInicio <= $queryFechaFin)){ 
            $data['ventas'] = $ventaModel->select('venta_cabecera.*, usuario.apellido, usuario.nombre')->join('usuario', 'usuario.id_usuario = venta_cabecera.id_usuario')->where('fecha >=', $queryFechaInicio)->where('fecha <', $fechaFinMasUno)->where('venta_cabecera.id_usuario', $userId)->orderBy('fecha', 'ASC')->paginate(7);

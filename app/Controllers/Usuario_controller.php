...
class Usuario_controller extends Controller{
    ...
    public function validarDatosUsuario(IncomingRequest $datos){
        ...
        $perfilModel = new Perfil_model();
        if(!$perfilModel->validarPerfil((int) $datos->getVar('perfil'))){
            $this->validator->setError('perfil', 'El Perfil seleccionado no es válido.');
            $valido = false;
        }

        $direccionModel = new Direccion_model();
        if(!$direccionModel->validarDireccion($datos->getVar('barrio'), $datos->getVar('calle'), (int) $datos->getVar('numero'))){
            $valido = false;
        }

        $localidadModel = new Localidad_model();
        if(!$localidadModel->validarLocalidad((int) $datos->getVar('localidad'), (int) $datos->getVar('provincia'))){
